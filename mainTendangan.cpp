...
void keyboard(unsigned char key, int mouseX, int mouseY) {
    switch (key) {
        ...
        case 32: // spacebar reset
            ...
            rotKaki = 0; kick = 0; roll = 0; touch = 0;
            break;
        case 27: // escape
            exit(0);
        default:
...
void Balok(float panjang, float lebar, float tinggi) {
    ...
    // depan
    glBegin(GL_QUADS);
    glVertex3f(-p, 0, l); glVertex3f(p, 0, l); glVertex3f(p, -t * 2, l); glVertex3f(-p, -t * 2, l);
    glEnd();
    // belakang
    glBegin(GL_QUADS);
    glVertex3f(-p, 0, -l); glVertex3f(p, 0, -l); glVertex3f(p, -t * 2, -l); glVertex3f(-p, -t * 2, -l);
