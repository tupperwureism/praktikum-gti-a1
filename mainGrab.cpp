...
int kick = 0, roll = 0, hit = 0, gerakTangan = 0, tabrak = 0;
int drop = 0, bring = 0, grab = 0;
float jarak = 1;
...
void pressKey(int k, int mouseX, int mouseY) {
    switch (k) {
...
void Balok(float panjang, float lebar, float tinggi) {
    ...
    glBegin(GL_QUADS); glVertex3f(-p, 0, l); glVertex3f(p, 0, l); glVertex3f(p, -t * 2, l); glVertex3f(-p, -t * 2, l); glEnd();
    glBegin(GL_QUADS); glVertex3f(-p, 0, -l); glVertex3f(p, 0, -l); glVertex3f(p, -t * 2, -l); glVertex3f(-p, -t * 2, -l); glEnd();
...
void pengubahTangan() {
    ...
    if (gerakTangan == 1) { // kondisi pelebarkan tangan samping
        if (rotTangan1 >= -90) rotTangan1 -= 0.1;
        if (rotTangan1 < -90) gerakTangan = 2;
    }
    if (gerakTangan == 2) { // kondisi merapatkan tangan depan
        if (rotTangan2 >= -90) rotTangan2 -= 0.1;
        if (rotTangan2 < -90 && hit == 1) grab = 1;
    }
    
    // melepas
    if (gerakTangan == 3) { // kondisi melebarkan tangan depan
        if (rotTangan2 <= 0) rotTangan2 += 0.1;
        if (rotTangan2 > 0) gerakTangan = 4;
    }
    if (gerakTangan == 4) { // kondisi merapatkan tangan samping
        if (rotTangan1 <= 0) rotTangan1 += 0.1;
        if (rotTangan1 > 0) gerakTangan = 0;
    }
}
