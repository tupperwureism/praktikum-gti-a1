...
void orientMe(float ang) {
    // Fungsi memutar arah kamera (tengok kiri/kanan)
    lx = sin(ang);
    lz = -cos(ang);
...
void moveMeFlat(int i) {
    // Fungsi gerak maju mundur kamera
    x = x + i * (lx) * 0.1;
    z = z + i * (lz) * 0.1;
...
void KotakKayu() {
    ...
    // Sisi belakang (dengan Texture Coordinate)
    glPushMatrix();
        glRotated(-180, 1, 0, 0);
        glTranslatef(0, 0, 3);
        glBegin(GL_QUADS);
    ...
    // Sisi bawah (dengan Texture Coordinate)
    glPushMatrix();
        glRotated(90, 1, 0, 0);
        glTranslatef(0, 0, 3);
        glBegin(GL_QUADS);
...
void pressKey(int key, int x, int y) {
    // Menangani penekanan tombol panah khusus
    switch (key) {
...
void releaseKey(int key, int x, int y) {
    // Menghentikan pergerakan saat tombol dilepas
    switch (key) {
...
int main(int argc, char **argv) {
    ...
    glutCreateWindow("3D Lighting and Camera");
